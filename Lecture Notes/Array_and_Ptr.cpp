...
//   種暫時物件，是右值表示式。
// * const T* ptr的意思是有一個名稱(字)叫ptr(向右沒有東西，再向左碰到*代表指標)的指標(再向左，碰到const T)，
//   指向const T，因此ptr指向的T物件不能被更改，但是ptr本身可以指向另一個不能被更動的T物件，因此更改ptr
//   儲存的位址是可以的(i.e. 但*ptr = t是不行的，但ptr++是可以的)。
// * T* const ptr的意思是有一個名稱(字)叫ptr(向右沒有東西，再向左碰到* const代表const指標)的const指標，
//   (再向左)指向T，因此ptr本身儲存的位址不能夠被改變，也就是說無法更動指向的記憶體位址，但是可以透過ptr更改
//   其所指向的T物件的內容值(i.e. ptr++是不行的，但*ptr = t是可以的)。
// * 記憶體位址的特性：
...
int main() {
    ...
    int v[10] = {0};  // * C語言規定，照理來說，要給10個元素，但這裡只給一個元素，
                      ...
                      // 其他元素則是自動補成0。
    int w[10] = {};   // * C++ 的歸零寫法。新版C++可省略等號: int w[10] {};
